...
task 'perform_release' do
  ...
  in_dir(WORKSPACE_DIR) do
    ...
    stage('PatchChangelog', 'Patch the changelog to update from previous release') do
      ...
      header = "### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/gwt-keycloak/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/realityforge/gwt-keycloak/compare/#{from}...v#{ENV['PRODUCT_VERSION']})"
      header += "\n"
      header += <<CONTENT
...
CONTENT
    ...
    stage('PatchReadme', 'Update the version in the README') do
      setup_filename = 'README.md'
      ...
      sh 'git reset 2>&1 1> /dev/null'
      sh "git add #{setup_filename}"
      sh "git commit -m \"Update README to reflect the #{ENV['PRODUCT_VERSION']} release\""
    end
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/gwt-keycloak', tag, :name => tag, :body => changes, :draft => false, :prerelease => true)

      candidates = client.list_milestones('realityforge/gwt-keycloak').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/gwt-keycloak', milestone[:number], :state => 'closed')
        end
      end
    end
  end
